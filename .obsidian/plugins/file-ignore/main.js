...
var FileOperations = class {
  constructor(vault) {
    ...
    this.debouncedGetFilesToProcess = (0, import_obsidian2.debounce)(
      (rules, resolve, reject) => {
...
var FileIgnoreSettingTab = class extends import_obsidian3.PluginSettingTab {
  constructor(app, plugin) {
    ...
    this.saveSettings = (0, import_obsidian4.debounce)(async (value) => {
    ...
    this.debouncedUpdateDisplay = (0, import_obsidian4.debounce)(() => {
      this.updateDisplayAsync();
    }, 600, false);
    this.plugin = plugin;
    this.t = this.plugin.t;
    this.debouncedUpdateMatchedFiles = (0, import_obsidian4.debounce)(
      () => {
        this.updateDisplayAsync();
      },
      1500
  ...
  loadRuleAndRefresh(ruleText, newIndex) {
    ...
    new import_obsidian3.Notice(this.t.notice.ruleLoadedFromHistory || "Rule loaded from history.");
  }
  ...
  async display() {
    ...
    new import_obsidian3.ButtonComponent(buttonGroup).setButtonText(this.t.applyRules.button).setCta().setTooltip(this.t.applyRules.desc).onClick(async () => {
      await this.plugin.applyRules(true);
      this.updateDisplayAsync();
    });
    new import_obsidian3.ButtonComponent(buttonGroup).setButtonText(this.t.revertRules.button).setTooltip(this.t.revertRules.desc).onClick(async () => {
      await this.plugin.applyRules(false);
      this.updateDisplayAsync();
    });
    new import_obsidian3.Setting(containerEl).setName(this.t.ignoreRules.title).setHeading();
    ...
    new import_obsidian3.ButtonComponent(titleActionContainer).setButtonText("Search").setCta().onClick(async () => {
      var _a;
      ...
      new import_obsidian3.Notice(((_a = this.t.notice) == null ? void 0 : _a.rulesAppliedAndScanned) || "Rules scanned and preview updated!");
    });
    ...
    this.prevAppliedRuleButton = new import_obsidian3.ButtonComponent(navContainer).setIcon("arrow-left").setTooltip(this.t.ignoreRules.previousAppliedRuleTooltip || "Previous applied rule").setClass("file-ignore-nav-button").onClick(() => {
      this.loadPreviousAppliedRule();
    });
    this.nextAppliedRuleButton = new import_obsidian3.ButtonComponent(navContainer).setIcon("arrow-right").setTooltip(this.t.ignoreRules.nextAppliedRuleTooltip || "Next applied rule").setClass("file-ignore-nav-button").onClick(() => {
      this.loadNextAppliedRule();
    });
    new import_obsidian3.ButtonComponent(navContainer).setButtonText("Reset").setTooltip("Restore default rules").setClass("file-ignore-nav-button").onClick(() => {
    ...
    this.rulesTextArea = new import_obsidian3.TextAreaComponent(textAreaContainer);
    ...
    this.updateDisplayAsync();
    const supportSetting = new import_obsidian3.Setting(containerEl).setName("Support the Developer").setDesc("If you find this plugin helpful, consider supporting its development.").addButton((button) => button.setButtonText("Buy Me a Coffee").onClick(() => {
  ...
  renderMatchedFilesList(container, matchedFiles, hiddenCount, error) {
    ...
    filesToRender.forEach((file) => {
      ...
      fileItem.addEventListener("click", () => {
        if (isFolder) {
          new import_obsidian3.Notice(`Folder: ${file.path}`);
        } else {
...
var FileIgnorePlugin = class extends import_obsidian5.Plugin {
  initTranslations() {
        ...
        this.app.workspace.on("file-menu", (menu, file) => {
          if (file instanceof import_obsidian5.TFile || file instanceof import_obsidian5.TFolder) {
            menu.addItem((item) => {
              ...
              item.setTitle(isHidden ? this.t.menu.show : this.t.menu.hide).setIcon(isHidden ? "eye" : "eye-off").onClick(async () => {
                if (isHidden) {
  ...
  async addDotPrefix(file) {
    ...
    try {
      ...
      if (fileInfo) {
        ...
        const itemType = file instanceof import_obsidian5.TFolder ? this.t.notice.folder : this.t.notice.file;
        new import_obsidian5.Notice(this.t.notice.hidden.replace("{itemType}", itemType));
        this.app.workspace.requestSaveLayout();
      }
    } catch (error) {
      console.error("\u9690\u85CF\u5931\u8D25:", error);
      new import_obsidian5.Notice(this.t.notice.hideError);
    }
  }
  async removeDotPrefix(file) {
    ...
    try {
      ...
      if (fileInfo) {
        ...
        const itemType = file instanceof import_obsidian5.TFolder ? this.t.notice.folder : this.t.notice.file;
        new import_obsidian5.Notice(this.t.notice.shown.replace("{itemType}", itemType));
        this.app.workspace.requestSaveLayout();
      }
    } catch (error) {
      console.error("\u663E\u793A\u5931\u8D25:", error);
      new import_obsidian5.Notice(this.t.notice.showError);
    }
  }
  async applyRules(hide) {
    if (!this.fileOps) {
      new import_obsidian5.Notice(this.t.notice.settingsErrorInit);
      return;
    }
    try {
      ...
      if (currentRulesArray.length === 0) {
        new import_obsidian5.Notice(this.t.notice.noRules);
        return;
      }
      const filesToProcess = await this.fileOps.getFilesToProcess(currentRulesArray);
      if (filesToProcess.length === 0) {
        new import_obsidian5.Notice(this.t.notice.noMatches);
        return;
      }
      let changedCount = 0;
      let result;
      for (const fileInfo of filesToProcess) {
        ...
        if (result) {
          if (result.success) {
            ...
            new import_obsidian5.Notice(`${this.t.notice[hide ? "hideError" : "showError"]} ${fileInfo.path}: ${errorMessage}`);
          }
        }
      }
      if (changedCount > 0) {
        new import_obsidian5.Notice(hide ? this.t.notice.applied(changedCount) : this.t.notice.reverted(changedCount));
        await this.saveSettings(currentRulesText, true);
      } else {
        new import_obsidian5.Notice(this.t.notice.noActionNeeded);
      }
    } catch (error) {
      console.error("[file-ignore] Error in applyRules:", error);
      new import_obsidian5.Notice(this.t.notice.applyError(error.message));
    }
  }
  async rollback() {
    var _a;
    try {
      ...
      new import_obsidian5.Notice(this.t.notice.rollbackSuccess);
      this.app.workspace.requestSaveLayout();
    } catch (error) {
      console.error("[file-ignore] Rollback operation failed:", error);
      new import_obsidian5.Notice(this.t.notice.rollbackError(error.message || this.t.notice.unknownError));
    }
  }
};
